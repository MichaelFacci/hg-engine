...
{
    debug_printf("nwav_preparebuffer\n");
    sInfo.bufPage = 0;
    update(&sInfo);
    update(&sInfo);
}
...
{
    debug_printf("nwav_stopinternal\n");
...
{
    debug_printf("nwav_updatecheckend\n");
...
{
    debug_printf("nwav_soundalarmhandler\n");
...
static void setup(void)
{
    ...
    debug_printf("setup: playRate=%d timerValue=%d bytesPerSample=%d alarmPeriod=%d\n",
        sInfo.playRate, timerValue, sInfo.bytesPerSample, alarmPeriod);
...
void NWAVPlayer_play(int fileID)
{
    if(strmThread == NULL)
	{
        ...
        debug_printf("strmThreadStack=%08X strmThread=%08X\n",
            (u32)strmThreadStack, (u32)strmThread);
        if (strmThreadStack == NULL || strmThread == NULL) {
            debug_printf("NWAV: heap 0 alloc FAILED for thread/stack\n");
        }
    ...
    debug_printf("hInfo magic=%08X rate=%d loopStart=%d loopEnd=%d fmt=%d evt=%d\n",
        hInfo.magic, hInfo.sampleRate, hInfo.loopStart, hInfo.loopEnd,
        hInfo.format, hInfo.numEvents);
    if (hInfo.magic != NWAV) {
        debug_printf("BAD NWAV MAGIC — wrong fileID or corrupt read\n");
    }
    ...
    if (pStrmBufL == NULL) {
        ...
        debug_printf("rawMemL=%08X\n", (u32)rawMemL);
        pStrmBufL = (u8*)(((u32)rawMemL + 31) & ~31);
        debug_printf("pStrmBufL=%08X\n", (u32)pStrmBufL);
    }
    /*
    if (sInfo.chCount > 1) {
...
static void StrmThread(void* arg)
{
    ...
    while (TRUE)
    {
        ...
        debug_printf("nwav_thread_woke msg=%08X\n", (u32)message);
